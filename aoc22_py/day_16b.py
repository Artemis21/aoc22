...
class Day(Solution):
    ...
    def max_pressure_release(self, start_valve: Valve, turns: int, closed_valves: list[Valve]) -> int:
        """Get the maximum pressure release from opening the given valves in any order."""
        max_release = 0
        for valve in closed_valves:
            this_turns = turns - start_valve.distances[valve.name] - 1
            if this_turns <= 0:
                continue
            this_closed_valves = [i for i in closed_valves if i != valve]
            release = this_turns * valve.flow_rate + self.max_pressure_release(valve, this_turns, this_closed_valves)
    ...
    def part_1(self) -> str | int | None:
        ...
        return self.max_pressure_release(start_valve, 30, valves)
    ...
    def part_2(self) -> str | int | None:
        ...
        start_valve = self.distances("AA")
        max_release = 0
        for valves_a, valves_b in self.partitions(valves):
            release_a = self.max_pressure_release(start_valve, 26, valves_a)
            release_b = self.max_pressure_release(start_valve, 26, valves_b)
            release = release_a + release_b
            if release > max_release:
                max_release = release
        return max_release
