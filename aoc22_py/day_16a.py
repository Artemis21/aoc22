...
class State:
    ...
    def lower_bound(self) -> int:
        ...
        for agent in self.agents:
            score = self.total_released
            turns = agent.turns
            last_valve = agent.valve
            for valve in self.closed_valves:
                turns -= last_valve.distances[valve.name] + 1
                if turns <= 0:
                    break
                score += valve.flow_rate * turns
                last_valve = valve
            scores.append(score)
        return max(scores)
    ...
    def upper_bound(self) -> int:
        ...
        if not self.agents:
            return self.total_released
        turns = max(i.turns for i in self.agents)
        if turns <= 2:
            return self.total_released
        ...
        for valve in self.closed_valves:
            costs: list[int] = []
            for agent in self.agents:
                cost = agent.valve.distances[valve.name] + 1
                if cost <= agent.turns:
                    costs.append(cost)
            if costs:
                total += valve.flow_rate * (turns - min(costs))
        return total
    ...
    def continuations(self) -> Iterable[State]:
        ...
        rest_agents = self.agents[1:]
        # for idx, agent in enumerate(self.agents):
        #     rest_agents = self.agents[:idx] + self.agents[idx + 1 :]
        for valve in self.closed_valves:
            turns = agent.turns - agent.valve.distances[valve.name] - 1
            if turns <= 0:
                continue
            new_agent = Agent(valve, turns)
            new_agents = [new_agent, *rest_agents]
