...
struct ItemActivity<const DIV: usize> {
    rounds: usize,
    monkey: usize,
    items: Vec<usize>,
    monkeys: Vec<Monkey>,
...
impl<const DIV: usize> ItemActivity<DIV> {
    fn activity(self) -> Vec<usize> {
        let mut activity = vec![0; self.monkeys.len()];
        for mut worry in self.items {
            let mut rounds = self.rounds;
            let mut monkey = self.monkey;
            while rounds > 0 {
                activity[monkey] += 1;
...
impl Day11 {
    fn run_rounds<const DIV: usize>(&self, rounds: usize) -> usize {
        let activity = self.items.iter()
            .enumerate()
            .map(|(monkey, items)| ItemActivity::<DIV> {
                rounds,
                monkey,
                items: items.clone(),
                monkeys: self.monkeys.clone(),
                worry_modulo: self.worry_modulo,
            })
            .map(|item| thread::spawn(move || item.activity()))
            .fold(
                vec![0; self.monkeys.len()],
                |mut activity, item_activity| {
                    for (monkey, &act) in item_activity.join().unwrap().iter().enumerate() {
                        activity[monkey] += act;
                    }
                    activity
                }
